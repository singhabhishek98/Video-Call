...
function Room() {
  ...
  const myMeeting = useCallback((type) => {
    if (!videoContainerRef.current) return; // Check for null before accessing
    const appID = Number(APP_ID); // Convert APP_ID to a number
  ...
  }, [roomId, navigate]); // Correct dependencies for useCallback
  ...
  }, [callType, myMeeting]); // Correct dependencies for useEffect
